...
interface DuaDhikirItem {
    ...
    arabic: string;
    source: string | null;
    image: string | null;       // raw storage path
    image_url: string | null;   // full display URL
...
export default function Edit({ item, categories }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Dashboard', href: '/dashboard' },
        { title: 'Dua & Dhikir', href: '/admin/dua-dhikir' },
        { title: `Edit #${item.id}`, href: `/admin/dua-dhikir/${item.id}/edit` },
    ];

    const fileInputRef = useRef<HTMLInputElement>(null);

    // Show existing image_url as initial preview
    ...
    const { data, setData, post, processing, errors } = useForm<{
        ...
        status: item.status,
        translations: item.translations.length > 0 ? [...item.translations] : [
            { language_code: 'en', title: '', translation: '', notes: '', benefits: '', fawaid: '' }
        ],
    });
    ...
    function updateTranslation(index: number, field: keyof Translation, value: string) {
        const newTranslations = [...data.translations];
        (newTranslations[index] as any)[field] = value;
        setData('translations', newTranslations);
    }
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Dua & Dhikir #${item.id}`} />
                ...
                <form onSubmit={submit} className="space-y-6" encType="multipart/form-data">
                    ...
                    <div className="flex items-center gap-3 py-4">
                        <Button type="submit" size="lg" disabled={processing}>
                            {processing ? 'Updating...' : 'Save All Changes'}
                        </Button>
